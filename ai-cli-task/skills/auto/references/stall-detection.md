...
Antigravity CLI (agy) may stall mid-execution (e.g., context window overflow prompt, waiting for user input, or internal hang). The daemon MUST actively detect and recover from stalls.
...
|---------------|----------------------|---------|
| < 3 | — | Normal (agy may be thinking/working) |
...
When stall is suspected, first verify the **foreground process** is agy itself — run `tmux list-panes -t '=${name}:' -F '#{pane_current_command}'` and confirm the result is `claude` (or `node` for Antigravity CLI). If the foreground process is a child process (e.g., `python`, `npm`, `gcc`), do NOT apply pattern matching recovery — the output belongs to a subprocess, not a stall. Only proceed with pattern matching when agy is the foreground process.
...
| Shell prompt visible | `$`, `>`, `%` at end of output (no active command) | Antigravity (agy) session ended unexpectedly -> restart auto session (see Server Recovery in main SKILL.md) |
