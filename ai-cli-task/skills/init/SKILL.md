...
```
/init <module_name> [--title "Task Title"] [--tags feature,backend] [--worktree]
```
...
- After init, the typical workflow is: edit `.target.md` → `/plan` → `/check` → `/exec`
