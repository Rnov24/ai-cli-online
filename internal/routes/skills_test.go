...
func TestSkillsHandler_ScaffoldSkill(t *testing.T) {
	...
	// 1. Invalid name
	badBody := bytes.NewBufferString(`{"name": "invalid name with spaces!", "scope": "workspace", "cwd": "` + tmpWorkspace + `"}`)
	reqBad := httptest.NewRequest("POST", "/api/skills/scaffold", badBody)
	...
	goodBody := bytes.NewBufferString(`{"name": "code-review", "description": "Automated code review runbook", "scope": "workspace", "cwd": "` + tmpWorkspace + `"}`)
	reqGood := httptest.NewRequest("POST", "/api/skills/scaffold", goodBody)
	...
	conflictBody := bytes.NewBufferString(`{"name": "code-review", "description": "Duplicate", "scope": "workspace", "cwd": "` + tmpWorkspace + `"}`)
	reqConflict := httptest.NewRequest("POST", "/api/skills/scaffold", conflictBody)
	...
	if wConflict.Code != http.StatusConflict {
		t.Errorf("expected 409 on duplicate skill, got %d", wConflict.Code)
	}
}
